...
- Aggregator: `ServerRegistry::add()`, `update()`, and `remove()` returned `update_option()`'s return value directly — but WordPress's `update_option()` returns `false` on both genuine failure AND no-op (when the new value equals the stored value). A user re-saving an unchanged server config, or removing a server in a state where the WP option already matched the desired result (concurrent edits, stale object cache invalidation), would surface as "Failed to delete server." / "Failed to add server." in the admin UI even though the desired state was achieved or already in place. All three methods now verify the post-condition by re-reading the option after `update_option` and ignore its return value. The test bootstrap's `update_option` stub now mirrors WP's no-op-returns-false behavior so this class of bug surfaces in tests instead of in production. (`newspack-event-aggregator/includes/class-server-registry.php`, `tests/bootstrap.php`, `tests/unit/ServerRegistryTest.php`)
...
- FlameBuilder: per-URL aggregate flame graph storage converted from EMA running means to additive sums, fixing the EMA-clamp distortion that caused flame timings to disagree with the bucketed leaderboard (e.g. "process" reading 59.6% while Total Profiled read 44.8% on the same URL detail view). The root flame's `count` is now unclamped, each child node carries `sum_value` and `seen_count`, and `finalize_flame_node` derives the displayed `value` as `sum_value / total_count` — mathematically equivalent to the previous "running mean × seen_count/total_count" formula but no longer dependent on the root count being clamped at `EMA_SAMPLE_LIMIT`. Per-URL flame data has a short TTL (~1h) and the worker resets the flame on first read of any pre-upgrade entry, so there's no migration step beyond restarting workers.
- Performance Dashboard: RequestProfile summary bar now uses wall clock (`totalMs`) as its denominator instead of profiled time, so the bar visually shows the unprofiled gap as empty background. Chunks sum to (profiledTime / totalMs) × 100% of bar width — matching the "Total Profiled" row's coverage percentage. Table row percentages updated to match (so % of Total in the table now reads as "% of wall clock", consistent with the bar widths and the Total Profiled row).
...
- Extracted shared `useTimeChart` hook — both AggregateTimeChart and CategoryTimeChart now share the same render/resize/scroll lifecycle, tooltip, legend, and time slot logic
- Hook category colors: lightened Query & Posts and The Events Calendar for readable text on flame graph bars
...
- Supervisor no longer skips spawning caught-up workers — all workers stay running continuously
