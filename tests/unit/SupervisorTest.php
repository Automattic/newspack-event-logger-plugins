...
class SupervisorTest extends TestCase {
	...
	public function test_worker_has_work_unknown_type(): void {
		$supervisor = new Supervisor();

		$ref = new \ReflectionMethod( Supervisor::class, 'worker_has_work' );
		$ref->setAccessible( true );

		$worker = [
			'type'      => 'unknown-reader',
			'partition' => 0,
			'lock_dir'  => self::TEST_DIR . '/locks/unknown.p0.lock.d',
		];

		// Unknown reader type should assume has work.
		$result = $ref->invoke( $supervisor, $worker, \time() );
		$this->assertTrue( $result );
	}

	public function test_worker_has_work_no_input_dir(): void {
		$supervisor = new Supervisor();

		// Inject a fake log_readers entry via reflection.
		$lr_ref = new \ReflectionProperty( Supervisor::class, 'log_readers' );
		$lr_ref->setAccessible( true );
		$lr_ref->setValue( $supervisor, [
			'fake-reader' => [
				'inputs'   => [ 'nonexistent.log' ],
				'handlers' => [],
			],
		] );
		...
		$worker = [
			'type'      => 'fake-reader',
			'partition' => 0,
			'lock_dir'  => self::TEST_DIR . '/locks/fake.p0.lock.d',
		];

		// No input partition directory = no work.
		$result = $ref->invoke( $supervisor, $worker, \time() );
	...
	// ── worker_has_work: input dir exists but caught up ─────────────────

	public function test_worker_has_work_caught_up(): void {
		$supervisor = new Supervisor();

		// Register a fake log reader with an input that has a partition directory.
		$lr_ref = new \ReflectionProperty( Supervisor::class, 'log_readers' );
		$lr_ref->setAccessible( true );
		$lr_ref->setValue( $supervisor, [
			'test-reader' => [
		...
		// Create the input partition directory with a segment.
		$logs_dir = Config::get_logs_directory();
		@\mkdir( "{$logs_dir}/firehose.log/p0", 0755, true );
		\file_put_contents( "{$logs_dir}/firehose.log/p0/0.log", 'test data' );

		// Create offsetlog with cursor at end of segment.
		$offsets_dir = Config::get_offsets_directory();
		$offsetlog   = new Firehose(
			"{$offsets_dir}/test-reader.p0",
			0,
		...
		$worker = [
			'type'      => 'test-reader',
			'partition' => 0,
			'lock_dir'  => self::TEST_DIR . '/locks/test.p0.lock.d',
		];

		$result = $ref->invoke( $supervisor, $worker, \time() );
		// Should be false (caught up since cursor is at or past end of segment).
		$this->assertFalse( $result );
	}

	// ── worker_has_work: input dir exists, cursor behind ────────────────

	public function test_worker_has_work_behind(): void {
		$supervisor = new Supervisor();
		...
		$logs_dir = Config::get_logs_directory();
		@\mkdir( "{$logs_dir}/firehose.log/p0", 0755, true );
		// Write enough data so cursor at 0 is clearly behind.
		\file_put_contents( "{$logs_dir}/firehose.log/p0/0.log", \str_repeat( 'x', 500 ) );
		...
		$result = $ref->invoke( $supervisor, $worker, \time() );
		// No offsetlog written, so cursor defaults to 0,0 -- definitely behind.
		$this->assertTrue( $result );
	}

	// ── worker_has_work: rate-limited catchup check uses cache ──────────

	public function test_worker_has_work_uses_cached_result(): void {
		$supervisor = new Supervisor();
		...
		$logs_dir = Config::get_logs_directory();
		@\mkdir( "{$logs_dir}/firehose.log/p0", 0755, true );
		\file_put_contents( "{$logs_dir}/firehose.log/p0/0.log", 'data' );

		// Set cached result and last check time to now (within CATCHUP_CHECK_INTERVAL).
		$cache_ref = new \ReflectionProperty( Supervisor::class, 'caught_up_cache' );
		$cache_ref->setAccessible( true );
		$cache_ref->setValue( $supervisor, [ 'cached-reader|0' => true ] );

		$check_ref = new \ReflectionProperty( Supervisor::class, 'last_catchup_check' );
		$check_ref->setAccessible( true );
		$check_ref->setValue( $supervisor, [ 'cached-reader|0' => \time() ] );
		...
		$worker = [
			'type'      => 'cached-reader',
			'partition' => 0,
			'lock_dir'  => self::TEST_DIR . '/locks/cached.p0.lock.d',
		];

		// Should use cached value (caught_up=true means no work).
		$result = $ref->invoke( $supervisor, $worker, \time() );
		$this->assertFalse( $result, 'Should use cached caught_up=true result' );
	}
	...
	// ── worker_has_work: empty segments in input ────────────────────────

	public function test_worker_has_work_empty_segments(): void {
		$supervisor = new Supervisor();
		...
		// Create partition dir but no segment files.
		$logs_dir = Config::get_logs_directory();
		@\mkdir( "{$logs_dir}/empty.log/p0", 0755, true );

		$ref = new \ReflectionMethod( Supervisor::class, 'worker_has_work' );
		$ref->setAccessible( true );

		$worker = [
			'type'      => 'empty-seg-reader',
			'partition' => 0,
			'lock_dir'  => self::TEST_DIR . '/locks/empty-seg.p0.lock.d',
		];

		// Input dir exists but no segments -> caught up (no data).
		$result = $ref->invoke( $supervisor, $worker, \time() );
		$this->assertFalse( $result );
	}

	// ── worker_has_work: multiple inputs all caught up ──────────────────

	public function test_worker_has_work_all_inputs_caught_up(): void {
		$supervisor = new Supervisor();
		...
		$logs_dir = Config::get_logs_directory();
		@\mkdir( "{$logs_dir}/a.log/p0", 0755, true );
		@\mkdir( "{$logs_dir}/b.log/p0", 0755, true );
		\file_put_contents( "{$logs_dir}/a.log/p0/0.log", 'aaaa' );
		\file_put_contents( "{$logs_dir}/b.log/p0/0.log", 'bbbb' );

		// Save positions at end of both segments.
		$offsets_dir = Config::get_offsets_directory();
		$offsetlog   = new Firehose(
			"{$offsets_dir}/multi-caught.p0",
			0,
			LogReader::OFFSETLOG_SEGMENT_SIZE,
			LogReader::OFFSETLOG_NUM_SEGMENTS,
			0
		...
		$worker = [
			'type'      => 'multi-caught',
			'partition' => 0,
			'lock_dir'  => self::TEST_DIR . '/locks/multi-caught.p0.lock.d',
		];

		$result = $ref->invoke( $supervisor, $worker, \time() );
		$this->assertFalse( $result, 'Should be false when all inputs are caught up' );
	}

	// ── worker_has_work: multiple inputs, one has no data ───────────────

	public function test_worker_has_work_partial_inputs(): void {
		$supervisor = new Supervisor();
		...
		$logs_dir = Config::get_logs_directory();
		// Only create one input directory.
		@\mkdir( "{$logs_dir}/firehose.log/p0", 0755, true );
		\file_put_contents( "{$logs_dir}/firehose.log/p0/0.log", 'data' );
		// requests.log/p0 does NOT exist.
		...
		$worker = [
			'type'      => 'multi-input',
			'partition' => 0,
			'lock_dir'  => self::TEST_DIR . '/locks/multi.p0.lock.d',
		];

		// At least one input exists, so should have work.
		$result = $ref->invoke( $supervisor, $worker, \time() );
		$this->assertTrue( $result );
	}
