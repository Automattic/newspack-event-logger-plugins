 ...
 *
 * Handles configuration loading and path validation.
 *
...
class Config {
	 ...
	 *
	 * Plugins can add their core options via the 'newspack_event_logger_option_schema_core' filter.
	...
	private static function get_option_schema_core(): array {
		...
		if ( \function_exists( 'apply_filters' ) ) {
			$schema = \apply_filters( 'newspack_event_logger_option_schema_core', $schema );
		}
	 ...
	 *
	 * Plugins can add their extended options via the 'newspack_event_logger_option_schema_extended' filter.
	...
	private static function get_option_schema_extended(): array {
		// Event Logger's own extended options.
		$schema = [
		...
		if ( \function_exists( 'apply_filters' ) ) {
			$schema = \apply_filters( 'newspack_event_logger_option_schema_extended', $schema );
		}
	...
	public static function load_config( string $mode = 'core' ): array {
		...
		if ( \defined( 'ABSPATH' ) && \function_exists( 'get_option' ) ) {
			...
			foreach ( self::get_option_schema_core() as $key => $type ) {
				$value = \get_option( "event_logger_{$key}" );
	...
	public static function load_config_defaults(): array {
		...
		$local_config_file = \getenv( 'LOCAL_EVENT_LOGGER_CONF' );
	 ...
	 *
	 * This method applies the newspack_event_logger_custom_colors filter lazily,
	...
	public static function get_custom_colors(): array {
		...
		if ( \function_exists( 'apply_filters' ) ) {
			$colors = \apply_filters( 'newspack_event_logger_custom_colors', $colors );
